...
resource "aws_instance" "hyperflowmaster" {
  ...
  user_data = "#!/bin/bash\necho ECS_CLUSTER=${var.ecs_cluster_name} >> /etc/ecs/ecs.config && docker run --net=host -p 3001:3001 -e DISK_DEVICE='xvda1' -e INTERFACE='eth0' -e METRIC_COLLECTOR='${var.influx_db_url}'  ${var.ec2_status_reporter} &"
}
